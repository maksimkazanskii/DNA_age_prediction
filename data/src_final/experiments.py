...
class DamagePredictor:
    ...
    def get_metadata(self, bam_name):
        ...
        if matching_rows.empty:
            ...
            if 'Age 1950 (rounded)' in matching_rows.columns:
                age = float(matching_rows['Age 1950 (rounded)'].str.replace(',', '').iloc[0])
                batch_name = matching_rows['Batch'].values[0]
            else:
                age = float(matching_rows['age'])
                batch_name = random.randint(0, 1000000)
    ...
    def prepare_dataset(self, basic_features=True, verbose=True):
        ...
        if verbose:
            ...
            print("\n The number of mutation type for each BAM file : ", len(features.columns))
    ...
    def partial_data_model(self, df, local_folder):
        ...
        for part in percentage_arr:
            ...
            pca_arr = []
            for item in self.CONFIG['pca_arr']:

                if item < part / 100 * 150:
                    pca_arr.append(item)
            pca_mae_arr = []
            pca_mae_std_arr = []
            for n_components in pca_arr:
                ...
                mae = []
                for i in range(0, 10):
                    n = round(part / 100.0 * len(df))
                    df_partial = df.sample(n=n)
            ...
            min_pca_mean = pca_mae_arr[min_value_index]  # Minimum value from pca_arr_mean
    ...
    @staticmethod
    def create_random_and_average_regressor(df, local_folder):
        ...
        # Random regressor evaluation
        mae_arr_random = []
        for i in range(0, 10):
            df_perm['permuted_label'] = np.random.permutation(df_perm['label'])
            df_perm['difference'] = (df_perm['permuted_label'] - df_perm['label']).abs()
            mae_random = df_perm['difference'].mean()
            mae_arr_random.append(mae_random)

        mae_random = np.array(mae_arr_random).mean()

        # Average regressor evaluation (constant prediction)
        average_label = df['label'].mean()
        df['average_label'] = average_label
        df['difference'] = (df['average_label'] - df['label']).abs()
        mae_average = df['difference'].mean()

        # Save both results to the same CSV
    ...
    def evaluate_models_with_pca(self, df, CONFIG, local_folder,seed = 42, save_model=True, verbose=True):
        ...
        y = df['label']
        groups = df['batch_name']
        print("Length of groups before combat:", len(groups))
        corrected_X = X
        ...
        for n_components in CONFIG['pca_arr']:
            ...
            for model_name, model, params in models:
                ...
                grid.fit(corrected_X, y, groups=groups)  # Use corrected data here
                best_pipeline = grid.best_estimator_
                y_pred = cross_val_predict(best_pipeline, corrected_X, y, cv=kf, groups=groups)
                ...
                mae_scores = cross_val_score(best_pipeline, corrected_X, y, cv=kf, groups=groups,
                ...
                result_arr.append({
                    'model': model_name,
                    'mae': mae, 'mae_std': mae_scores.std(),
                    'rmse': rmse, 'rmse_std': rmse_scores.std(),
                    'r2': r2, 'r2_std': r2_scores.std()
                })
                ...
                if verbose:
                    print(f"Model: {model_name} | PCA components: {n_components} | "
                          f"MAE: {mae:.4f} ± {mae_scores.std():.4f} | "
                          f"RMSE: {rmse:.4f} ± {rmse_scores.std():.4f} | "
                          f"R2: {r2:.4f} ± {r2_scores.std():.4f}")
...
if __name__ == '__main__':
    ...
    DP.create_random_and_average_regressor(df_features, local_folder='RANDOM_REGRESSOR')
